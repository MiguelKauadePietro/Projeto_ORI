...
Adicione as imagens da apresentacao em uma pasta `docs/images/` e substitua os
campos abaixo pelos arquivos correspondentes.

### Visualizacao da Quadtree

```text
[Inserir print do mapa com os quadrantes]
```

### Visualizacao por Municipio

```text
[Inserir print dos marcadores municipais]
```

### Visualizacao por Pontos

```text
[Inserir print dos registros por CEP/coordenadas]
```

### Visualizacao de Concentracao

```text
[Inserir print dos circulos proporcionais]
```

## Relacao com a Disciplina de ORI

O projeto aplica conceitos de Organizacao e Recuperacao da Informacao ao tratar
dados estatisticos georreferenciados. Antes de serem exibidos, os dados passam
por leitura, validacao, normalizacao, indexacao espacial e agregacao.

A Quadtree atua como uma estrutura de organizacao que associa cada registro a
uma regiao do espaco. Essa organizacao permite recuperar apenas os pontos de uma
area consultada e obter resumos estatisticos sem tratar todos os registros como
uma lista sem estrutura.

O projeto tambem permite comparar diferentes formas de recuperar informacao:

- por unidade administrativa, usando municipios;
- por registro individual, usando CEP ou coordenadas;
- por concentracao de ocorrencias;
- por particionamento espacial hierarquico, usando Quadtree.

Assim, a aplicacao demonstra como a escolha da estrutura de dados influencia a
eficiencia, a granularidade e a forma de apresentar a informacao recuperada.

## Melhorias Futuras

- Receber upload de arquivos CSV pelo frontend.
- Permitir diferentes datasets e indicadores na mesma execucao.
- Usar GeoJSON para colorir os limites reais dos municipios.
- Integrar codigos oficiais e centroides do IBGE.
- Converter CEPs sem coordenadas por meio de uma base geografica local.
- Adicionar clustering para grandes quantidades de pontos proximos.
- Consultar a Quadtree por area visivel (`bbox`) e retornar somente os nos
  relevantes.
- Considerar zoom e pan na consulta espacial do backend.
- Persistir dados em PostgreSQL com PostGIS ou outro banco espacial.
- Armazenar a Quadtree em cache para evitar reconstrucoes repetidas.
- Adicionar novos criterios de agregacao e escalas de cores.
- Criar testes de integracao para todos os endpoints.
- Medir tempo de construcao, consulta e quantidade de elementos renderizados.
- Adicionar acessibilidade e layout aprimorado para dispositivos moveis.

## Licenca

Projeto academico distribuido sob a licenca MIT, conforme definido no
`package.json`.
